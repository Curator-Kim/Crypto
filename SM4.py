...
def encrypt(data,key):#加密主函数
    x=[0]*4
    x[0],x[1],x[2],x[3]=data>>96,data>>64&0xffffffff,data>>32&0xffffffff,data&0xffffffff
    key=keyGenerator(key)
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[4])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[5])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[6])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[7])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[8])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[9])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[10])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[11])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[12])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[13])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[14])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[15])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[16])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[17])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[18])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[19])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[20])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[21])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[22])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[23])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[24])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[25])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[26])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[27])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[28])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[29])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[30])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[31])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[32])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[33])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[34])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[35])
    cipher=(x[3]<<96)|(x[2]<<64)|(x[1]<<32)|x[0]
    return cipher
...
def decrypt(data,key):#解密主函数
    ...
    x[0],x[1],x[2],x[3]=data>>96,data>>64&0xffffffff,data>>32&0xffffffff,data&0xffffffff
    key=keyGenerator(key)
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[35])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[34])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[33])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[32])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[31])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[30])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[29])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[28])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[27])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[26])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[25])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[24])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[23])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[22])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[21])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[20])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[19])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[18])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[17])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[16])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[15])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[14])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[13])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[12])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[11])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[10])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[9])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[8])
    x[0] = x[0] ^ T(x[1] ^ x[2] ^ x[3] ^ key[7])
    x[1] = x[1] ^ T(x[2] ^ x[3] ^ x[0] ^ key[6])
    x[2] = x[2] ^ T(x[3] ^ x[0] ^ x[1] ^ key[5])
    x[3] = x[3] ^ T(x[0] ^ x[1] ^ x[2] ^ key[4])
    cipher=(x[3]<<96)|(x[2]<<64)|(x[1]<<32)|x[0]
    return cipher
    
def keyGenerator(key):#产生密钥
    k=[0]*36
    k[0],k[1],k[2],k[3]=key>>96,key>>64&0xffffffff,key>>32&0xffffffff,key&0xffffffff
    k[0],k[1],k[2],k[3]=k[0]^0xA3B1BAC6,k[1]^0x56AA3350,k[2]^0x677D9197,k[3]^0xB27022DC
    for i in range(4,36):
        k[i]=k[i-4]^T_key(k[i-3]^k[i-2]^k[i-1]^CK[i-4])
    return k

def T(data):#论函数中所需要的T函数
    a0,a1,a2,a3=data>>24,(data>>16)&0xff,(data>>8)&0xff,data&0xff
    b0,b1,b2,b3=T_table4[a0 >> 4][a0 & 0xf],T_table3[a1 >> 4][a1 & 0xf],T_table2[a2 >> 4][a2 & 0xf],T_table1[a3 >> 4][a3 & 0xf]
    data=b0^b1^b2^b3
    return data

def sbox_subtitute(data):#字节代替
    return sbox[data>>4][data&0xf]

def move_left(data,bit):#左移函数
    data=((data<<bit)&0xffffffff)|(data>>(32-bit))
    return data

def T_key(key):#产生密钥所需要的T函数
    a0,a1,a2,a3=key>>24,key>>16&0xff,key>>8&0xff,key&0xff
...
def test(data):#测试输出函数
    for i in range(len(data)):
        print(hex(data[i]))

if __name__=='__main__':
    print(hex(encrypt(0x0123456789abcdeffedcba9876543210,0x0123456789abcdeffedcba9876543210)))
    print(hex(decrypt(0x681edf34d206965e86b3e94f536e4246,0x0123456789abcdeffedcba9876543210)))
    print(hex(encrypt(0x30313233343536373839616263646566,0x0123456789abcdeffedcba9876543210)))
    print(hex(decrypt(0xe6887b77dbabb572ffa07fed7548b192,0x0123456789abcdeffedcba9876543210)))
    print(hex(encrypt(0x0123456789abcdeffedcba9876543210,0x2136410abd7ce2f49210af5ce4b39afb)))
    print(hex(decrypt(0xae6488917ef4ff6c68d3dcf20301ca91,0x2136410abd7ce2f49210af5ce4b39afb)))
    # print(hex(encrypt(0x010f0f0f0f0f0f0f0f0f0f0f0f0f0f0f,0x0123456789abcdeffedcba9876543210)))
